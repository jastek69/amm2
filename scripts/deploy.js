...
async function main() {
  ...
  let sob = await Token.deploy('Sobek', 'SOB', '1000000') // 1 million tokens
  await sob.deployed()  
  console.log(`Sobek Token deployed to: ${sob.address}\n`)
  ...
  const amm = await AMM.deploy(sob.address, usd.address)
